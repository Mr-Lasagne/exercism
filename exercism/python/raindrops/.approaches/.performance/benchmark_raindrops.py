...
multiple approaches to the Raindrops problem in Python.
...
def convert_sequence_join(number):
    ...
    factors = [3, 5, 7]
    sounds = "".join(
        drops[index] for index, factor in enumerate(factors) if (number % factor == 0)
    )
...
def convert_dict(number):
    ...
    results = "".join(
        sounds[divisor] for divisor, sound in sounds.items() if number % divisor == 0
    )
