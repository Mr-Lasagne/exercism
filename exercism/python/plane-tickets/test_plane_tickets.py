...
class TestPlaneTickets(unittest.TestCase):
    ...
    def test_task1_returns_generator(self):
        ...
        number = 5
        error_message = (
            f"Called generate_seat_letters({number}). "
        ...
        self.assertTrue(
            inspect.isgenerator(generate_seat_letters(number)), msg=error_message
        )
    ...
    def test_generate_seat_letters(self):
        ...
        for variant, (number, expected) in enumerate(
            ...
            with self.subTest(
                f"variation #{variant}", number=number, expected=expected
            ):
    ...
    def test_task2_returns_generator(self):
        ...
        number = 7
        error_message = (
            f"Called generate_seats({number}). "
    ...
    def test_assign_seats(self):
        ...
        for variant, (passengers, expected) in enumerate(
            zip(test_data, result_data), start=1
        ):
            with self.subTest(
                f"variation #{variant}", passengers=passengers, expected=expected
            ):
                actual_result = assign_seats(passengers)
                error_message = (
                    f"Called assign_seats({passengers}). "
                    f"The function returned {actual_result}, but the tests "
    ...
    def test_task4_returns_generator(self):
        ...
        error_message = (
            f"Called generate_codes({seat_numbers}, {flight_id}). "
            f"The function returned a {type(generate_codes(seat_numbers, flight_id))} type, "
            f"but the tests expected the function to return a <class 'generator'> type."
        )

        self.assertTrue(
            inspect.isgenerator(generate_codes(seat_numbers, flight_id)),
            msg=error_message,
        )
    ...
    def test_generate_codes(self):
        ...
        for variant, ((seat_numbers, flight_id), expected) in enumerate(
            zip(test_data, result_data), start=1
        ):
            with self.subTest(
                f"variation #{variant}",
                ...
                self.assertEqual(
                    list(generate_codes(seat_numbers, flight_id)),
                    expected,
                    msg=error_message,
                )
