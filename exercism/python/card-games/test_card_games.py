...
class TestCardGames(unittest.TestCase):
    ...
    def test_average_even_is_average_odd(self):
        ...
        for variant, (input_hand, expected) in enumerate(
            zip(input_data, result_data), start=1
        ):
            with self.subTest(
                f"variation #{variant}", input_hand=input_hand, expected=expected
            ):
    ...
    def test_maybe_double_last(self):
        ...
        for variant, (hand, expected) in enumerate(
            ...
            with self.subTest(
                f"variation #{variant}", hand=list(hand), expected=expected
            ):
