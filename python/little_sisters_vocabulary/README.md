# Little Sister's Vocabulary: Instruction
...
In a file called `little_sisters_vocabulary.py`, implement a program that
calculates various results related to editing English vocabulary homework. The
...
In a file called `test_little_sisters_vocabulary.py`, implement a test suite
that covers the following test cases:

```python
# Ensure that `add_prefix_un` returns correctly.
>>> add_prefix_un("happy")
'unhappy'

# Ensure that `make_word_groups` returns correctly.
>>> make_word_groups(["en", "close", "joy", "lighten"])
'en :: enclose :: enjoy :: enlighten'

# Ensure that `remove_suffix_ness` returns correctly.
>>> remove_suffix_ness("heaviness")
'heavy'
>>> remove_suffix_ness("sadness")
'sad'

# Ensure that `adjective_to_verb` returns correctly.
