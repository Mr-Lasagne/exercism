...
import pytest
from categories_data_cater_waiter import KETO, OMNIVORE, PALEO, VEGAN, VEGETARIAN
from cater_waiter import (
    categorize_dish,
    check_drinks,
    clean_ingredients,
    compile_ingredients,
...
from test_data_cater_waiter import (
    all_drinks,
...
class TestCaterWaiter(unittest.TestCase):
    ...
    def test_clean_ingredients(self):
        ...
        for variant, (item, result) in enumerate(test_data, start=1):
            with self.subTest(
                f"variation #{variant}",
                inputs="recipes with duplicated ingredients",
                ...
                self.assertEqual(
                    clean_ingredients(item[0], item[1]),
                    (result[1], result[2]),
                    msg=error_msg,
                )
    ...
    def test_check_drinks(self):
        ...
        for variant, (item, result) in enumerate(test_data, start=1):
            with self.subTest(
                f"variation #{variant}",
                inputs="all drinks",
                results="drinks classified",
            ):
                error_msg = (
                    f"Expected {result} for {item}, but got something else instead."
                )
                self.assertEqual(
    ...
    def test_categorize_dish(self):
        test_data = zip(
            sorted(recipes_without_duplicates, reverse=True)[::3],
            dishes_categorized[::3],
        )

        for variant, (item, result) in enumerate(test_data, start=1):
            with self.subTest(
                f"variation #{variant}",
                inputs="all recipes list",
                results="categorized dishes",
            ):
                error_message = f"Expected category {result} for {item[0]}, but got a different category instead."
                self.assertEqual(
                    categorize_dish(item[1], item[2]), (result), msg=error_message
                )
    ...
    def test_tag_special_ingredients(self):
        ...
        for variant, (item, result) in enumerate(test_data, start=1):
            with self.subTest(
                f"variation #{variant}",
                inputs="all recipes list",
                results="special ingredients tagged",
            ):
                error_message = (
                    f"Expected {result} for {item}, but got something else instead."
                )
                self.assertEqual(
                    tag_special_ingredients(item), (result), msg=error_message
                )
    ...
    def test_separate_appetizers(self):
        ...
        for variant, (item, result) in enumerate(test_data, start=1):
            with self.subTest(
                f"variation #{variant}",
                inputs="dishes with appetizers",
                results="appetizers only",
            ):
                ...
                self.assertIsInstance(
                    separate_appetizers(item[0], item[1]), list, msg=result_type_error
                )
                self.assertEqual(
                    sorted(separate_appetizers(item[0], item[1])),
                    (sorted(result)),
                    msg=error_message,
                )
    ...
    def test_singleton_ingredients(self):
        ...
        for variant, (item, result) in enumerate(test_data, start=1):
            with self.subTest(
                f"variation #{variant}",
                inputs="overlapping ingredients",
                results="ingredients in only one dish",
            ):
                error_message = (
                    "Expected only ingredients that belong to exactly "
                    "one dish, but got multi-dish ingredients instead."
                )
                self.assertEqual(
                    singleton_ingredients(item[0], item[1]), (result), msg=error_message
                )
