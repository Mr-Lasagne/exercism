...
try:
    from ellens_alien_game import Alien
except ImportError:
    raise ImportError(
        "\n\nMISSING CLASS --> We tried to import the 'Alien' class from "
        "your classes.py file, but could not find it."
        "Did you misname or forget to create it?"
    ) from None

try:
    from ellens_alien_game import new_aliens_collection
except ImportError:
    raise ImportError(
...
class TestEllensAlienGame(unittest.TestCase):
    ...
    def test_alien_has_correct_initial_coordinates(self):
        ...
        alien = Alien(2, -1)
        error_message = (
            f"Created a new Alien by calling Alien(2, -1). "
        ...
        self.assertEqual(
            (2, -1), (alien.x_coordinate, alien.y_coordinate), msg=error_message
        )
    ...
    def test_alien_has_health(self):
        alien = Alien(0, 0)
        error_message = (
            f"Created a new Alien by calling Alien(0, 0). "
    ...
    def test_alien_instance_variables(self):
        ...
        coord_x_error = (
            f"Created two new Aliens by assigning "
    ...
    def test_alien_hit_method(self):
        ...
        for variant, (iterations, expected) in enumerate(
            zip(test_data, result_data), start=1
        ):
            alien = Alien(2, 2)

            with self.subTest(
                f"variation #{variant}", iterations=iterations, expected=expected
            ):
                for _ in range(iterations):
                    alien.hit()

                error_message = (
                    f"Called hit() {iterations} time(s) "
    ...
    def test_alien_is_alive_method(self):
        ...
        alive_error = (
            "Created a new Alien and called hit(). "
    ...
    def test_alien_teleport_method(self):
        ...
        error_message = (
            "Called alien.teleport(-1,-4) on a newly created Alien. "
        ...
        self.assertEqual(
            (-1, -4), (alien.x_coordinate, alien.y_coordinate), msg=error_message
        )
    ...
    def test_alien_collision_detection_method(self):
        alien = Alien(7, 3)
        error_message = (
            "Created a new Alien at (7,3) and called "
    ...
    def test_alien_class_variable(self):
        ...
        created_error_message = (
            "Created two new Aliens and requested the "
        ...
        self.assertEqual(alien_two.health, alien_one.health, msg=health_error_message)
    ...
    def test_alien_total_aliens_created(self):
        ...
        error_message = (
            "Created a new Alien and called total_aliens_created for it. "
        ...
        def error_text(alien, variable):
            return (
                "Created two additional Aliens for the session."
        ...
        self.assertEqual(
            3, aliens[0].total_aliens_created, msg=error_text(1, aliens[0])
        )
        self.assertEqual(
            3, aliens[1].total_aliens_created, msg=error_text(2, aliens[1])
        )
        self.assertEqual(
            3, aliens[2].total_aliens_created, msg=error_text(3, aliens[2])
        )
    ...
    def test_new_aliens_collection(self):
        ...
        for position, obj in zip(test_data, actual_result):
            position_error = (
                f"After calling new_aliens_collection({test_data}), "
