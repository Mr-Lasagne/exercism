...
class TestElectricBill(unittest.TestCase):
    ...
    def test_get_extra_hours(self):
        ...
        for variant, (input_data, output_data) in enumerate(
            zip(input_data, output_data), start=1
        ):
            with self.subTest(
                f"variation #{variant}", input_data=input_data, output_data=output_data
            ):
        ...
        for variant, (input_data, output_data) in enumerate(
            ...
            with self.subTest(
                ...
                self.assertAlmostEqual(
                    get_efficiency(input_data), output_data, msg=error_msg
                )
