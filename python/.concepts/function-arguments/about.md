# Function Arguments <!-- omit in toc -->
...
`*args` is a two-part name that represents a `tuple` with an indefinite number
...
Note that when an argument is already in an iterable, such as a tuple or list,
